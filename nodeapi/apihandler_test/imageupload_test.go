...
//	mongodb.InitFileStreamDbClientWithConfig(gridDbTestConfigFile)
//	dbConfig := configurations.ReadDbConfig(gridDbTestConfigFile)
//	fmt.Println("testdatabase", dbConfig.DbName, dbConfig.ImageBucketName, "will be set empty")
//	ctx, _ := context.WithTimeout(context.Background(), 10*time.Second)
//	err := mongodb.GridMongoClient.Database(dbConfig.DbName).Collection(dbConfig.ImageBucketName + ".files").Drop(ctx)
//	err = mongodb.GridMongoClient.Database(dbConfig.DbName).Collection(dbConfig.ImageBucketName + ".chunks").Drop(ctx)
//	if err != nil {
//		log.Fatal(err)
//	}
//	return true
